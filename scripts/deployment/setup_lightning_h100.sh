...
info "Lightning H100 bootstrap starting"
...
if command -v nvidia-smi >/dev/null 2>&1; then
    ...
    if [[ "$GPU_NAME" != *"H100"* ]]; then
        warn "This script is optimized for H100, but current GPU is: $GPU_NAME"
    fi
else
...
Next recommended commands on Lightning H100:
...
ok "Lightning H100 bootstrap complete"